...
class Exemplar(BaseModel):
    def __init__(
        ...
        if catalogo is None:
            self.catalogo_ref = None
        elif isinstance(catalogo, CatalogoReference):
            self.catalogo_ref = catalogo
        else:
            # If a Catalogo object is passed, create a reference
            self.catalogo_ref = CatalogoReference(
                catalogo.id if hasattr(catalogo, 'id') else catalogo
            )
        ...
        if self.catalogo_ref is not None:
    ...
    def __repr__(self):
        ...
        return (
            ...
            f"id_catalogo={catalogo_id}, tipo={self.tipo_midia})>"
        )
