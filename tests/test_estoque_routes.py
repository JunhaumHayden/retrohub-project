...
from app.models import Funcionario, Usuario, Catalogo, Exemplar, MidiaFisica, MidiaDigital, Transacao, ItemTransacao
...
class TestEstoqueRoutes(unittest.TestCase):
    ...
    def test_4_listar_estoque_do_jogo(self):
        ...
        self.client.post('/api/estoque/fisico', json={"id_catalogo": self.jogo_id, "codigo_barras": "CT-01", "estado_conservacao": "RUIM"}, headers=self.headers)
        self.client.post('/api/estoque/digital', json={"id_catalogo": self.jogo_id, "chave_ativacao": "CT-DIG-01"}, headers=self.headers)
    ...
    def test_5_atualizar_estado_fisico(self):
        ...
        res_create = self.client.post('/api/estoque/fisico', json={"id_catalogo": self.jogo_id, "codigo_barras": "CT-UPDATE", "estado_conservacao": "NOVO"}, headers=self.headers)
    ...
    def test_6_exclusao_exemplar(self):
        ...
        res_create = self.client.post('/api/estoque/fisico', json={"id_catalogo": self.jogo_id, "codigo_barras": "CT-DELETE", "estado_conservacao": "PESSIMO"}, headers=self.headers)
        midia_id = res_create.get_json()['id']
        
        res_delete = self.client.delete(f'/api/estoque/{midia_id}', headers=self.headers)
        self.assertEqual(res_delete.status_code, 200)
        
        # Tenta buscar pelo ID do Catalogo e deve vir vazio
