...
elif ! curl -s -o /dev/null -w "%{http_code}" "$BASE/docs" | grep -q 200; then
  echo "ERRO: porta 5000 ocupada mas API não responde em $BASE/docs"
  exit 1
fi
...
R=$(request -X POST "$BASE/api/catalogo/itens/" \
  -H "Content-Type: application/json" -H "X-Funcionario-Id: $FUNCIONARIO_ID" \
  -d "{\"titulo\":\"$TITULO\",\"genero\":\"Aventura\",\"situacao\":\"DISPONIVEL\"}")
check "POST catalogo" "$(echo "$R" | tail -1)" "201"
CAT_ID=$(echo "$R" | sed '$d' | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])")
...
R=$(request -X POST "$BASE/api/vendas/solicitar" \
  -H "Content-Type: application/json" -H "X-Cliente-Id: $CLIENTE_ID" \
...
R=$(request -X PATCH "$BASE/api/alugueis/$ALUGUEL_ID/retirada" -H "X-Funcionario-Id: $FUNCIONARIO_ID")
...
R=$(request -X PATCH "$BASE/api/alugueis/$ALUGUEL_ID/devolucao" \
  -H "Content-Type: application/json" -H "X-Funcionario-Id: $FUNCIONARIO_ID" -d '{"condicao_item":"bom"}')
...
R=$(request "$BASE/api/avaliacoes/minhas" -H "X-Cliente-Id: $CLIENTE_ID")
check "GET minhas avaliacoes" "$(echo "$R" | tail -1)" "200"

echo ""
echo "6. Relatório"
R=$(request "$BASE/api/relatorios/compras-locacoes" -H "X-Funcionario-Id: $FUNCIONARIO_ID")
